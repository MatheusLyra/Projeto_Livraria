...
public class LivroDao {
	...
	public void adiciona(Livro livro) {
		String sql = "insert into livros1(titulo, autor, editora, email, dataLancamento) values (?,?,?,?,?)";
	...
	public List<Livro> getLista() {
		try {
			...
			String sql = "select * from livros1";
	...
	public void altera(Livro livro) {
		String sql = "update livros1 set titulo=?, autor=?, editora=?, email=?, dataLancamento=? where id=?";
	...
	public void exclui(Livro livro){
		try {
			String sql = "delete from livros1 where id=?";
			PreparedStatement stmt = conn.prepareStatement(sql);
